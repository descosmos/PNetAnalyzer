...
namespace PNetAnalyzer
{
    class Program
    {
        static void CaptureNetworkCommnunication()
        {
                ...
                if (data.ProcessID == targetPid)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (one.Count == 1 && two.Count == 0)
                            {
                                Console.WriteLine(String.Format("名称: {3}, 远程IP: {0}, 远程端口: {4}, 接收数据: {1}, 时间: {2}",
                                    one[0][0], one[0][1], one[0][2], one[0][3], one[0][4]));
                                one.Clear();
                                list1.Add(data.daddr.ToString()); //IP   0
                                list1.Add(data.size.ToString()); //大小  1
                                list1.Add(time); //时间   2
                            ...
                            while (one.Count != 1)
                            {
                                ...
                                Console.WriteLine(String.Format("名称: {3}, 远程IP: {0}, 远程端口: {4}, 接收数据: {1}, 时间: {2}",
                                    one[0][0], size, one[0][2], one[0][3], one[0][4]));
                                one.Clear();
                                if (two.Count != 0)
                                {
                                    one.AddRange(two);
            ...
            //     if (data.ProcessID == targetPid)
            //     {
            //         Console.WriteLine(String.Format("建立{0}",data.daddr));
            //     }
            //
            //     
            // };
        ...
        {
            string ipAddress = "36.110.162.63"; // 目标IP地址
            int timeoutMs = 1000; // 超时时间（毫秒）

            Ping pingSender = new Ping();
            PingReply reply = pingSender.Send(ipAddress, timeoutMs);

            if (reply.Status == IPStatus.Success)
            {
                Console.WriteLine("TCP网络延迟: {0}ms", reply.RoundtripTime);
            }
            else
            {
